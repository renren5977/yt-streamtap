...
def get_video_chunk_info(data, type: str="fmp4"):
    ...
    if type == "fmp4":
        data = parse_fmp4(data)
        return get_video_fmp4_init_info(data)

    if type == "webm":
        data = parse_webm(data)
        return get_video_webm_cluster_info(data)
    
def get_audio_chunk_info(data):
    def get_audio_webm_cluster_info(cluster: dict) -> dict:
        """
        parse_webm() 後の Cluster dict だけを受け取って、
        ts_start / ts_end を返す。
    ...
    data = parse_webm(data)
    return get_audio_webm_cluster_info(data["1F43B675"])
